...
- [x] Fechou mint JWT / PII debug ainda públicos: `test-token` (ADMIN), `admin/generate-token` (`CRON_SECRET`), `test-supabase-users` + `debug-supabase-auth` (ADMIN); `fix-token` sem mint ADMIN em token vazio/inválido; `fix-auth` com `verifyToken`; `test-users` GET só leitura; `execute-sql` sem CREATE SECURITY DEFINER
- [x] QHSE “Colaborador não encontrado”: `CATALOG_COLAB_SELECT` pedia `cargo_nome` em `gt_colaboradores` (alias da view) — feito em #98 (`51803cf9`) no `portal`
