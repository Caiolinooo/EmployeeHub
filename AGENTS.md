...
- **Documentos globais do colaborador**: catálogo vivo em `src/lib/document-catalog/` (`tax_id` / user id / e-mail / telefone / nome único). Nunca `SELECT cpf` em `users_unified`. Portal user existente não vira 404. Inclui QHSE/ficha EPI e listas de presença assinadas. Sem copiar blobs. Ver `src/lib/document-catalog/AGENTS.md`.
