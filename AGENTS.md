...
- **Mobile-first (2026-09)**: plano em `docs/mobile-first-plan.md` §11. Front mobile separado (`src/app/(mobile)/m/**`); desktop congelado. Rewrite no `middleware.ts` e fallback P0 em `next.config.js` `rewrites.beforeFiles` (UA + `Sec-CH-UA-Mobile` + cookie `ui`). Tablet (iPad e Android) = desktop. `ClientProviders` / `CompanionSessionContext` sem toque. Sem `src/app/login/layout.tsx` (reordena CSS e quebra `next/font` no desktop). Allowlist em `src/lib/mobile-ui/device-surface.ts`. Sem merge em `portal` até o dono pedir.
