...
- **Middleware (Next 15.5)**: arquivo único `src/middleware.ts`. Nunca criar `pages/` ou `app/` na raiz — `findDir` prioriza `./pages` e o build procura `middleware.*` em `path.join(pagesDir, '..')`, o que ignora `src/middleware.ts` e gera `.next/server/middleware-manifest.json` com `middleware: {}`. Verificar com `npm run assert:middleware`. Matcher atual exclui `/api` (auth fica no handler). Header de prova: `x-abz-middleware`.
