...
- Select de `gt_colaboradores` (`CATALOG_COLAB_SELECT`) só usa colunas reais da tabela + embed `cargo:gt_cargos(nome)`. **Nunca** `cargo_nome` — essa coluna existe só em `gt_vw_colaboradores_completo`; PostgREST falha e a aba QHSE / EPI devolve 404 `Colaborador não encontrado` mesmo com o id válido da ficha.
...
- `CATALOG_COLAB_SELECT` passa `catalogColabSelectIsSafe` (sem `cargo_nome`).
