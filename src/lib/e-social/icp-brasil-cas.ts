/**
 * Public ICP-Brasil root CAs (ITI). Not secrets.
 * Sources: https://acraiz.icpbrasil.gov.br/credenciadas/RAIZ/
 * - Autoridade Certificadora Raiz Brasileira v5 (valid until 2029-03-02)
 * - Autoridade Certificadora Raiz Brasileira v10 (valid until 2032-07-01)
 * PEM copies also live in ./certs/ for audit. Runtime uses these strings so
 * Vercel bundling does not depend on reading .pem from disk.
 */
export const ICP_BRASIL_V5_PEM = `-----BEGIN CERTIFICATE-----

0gv8bn5NdUX8UMNX1sHhU3H1UpoW
-----END CERTIFICATE-----`;

export const ICP_BRASIL_V10_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;

export const ICP_BRASIL_CA_PEMS: readonly string[] = [ICP_BRASIL_V5_PEM, ICP_BRASIL_V10_PEM];
