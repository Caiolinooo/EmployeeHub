...
- Regex de estrutura XML (`xml-linear.ts`): quantifiers bounded / non-overlapping. `validarXMLGerado` não usa `+`/`*` abertos em classes que se sobrepõem.
...
- `npx tsx --test src/lib/e-social/ts-nome.test.ts src/lib/e-social/esocialAutoCorrector.test.ts src/lib/e-social/esocial-date.test.ts src/lib/e-social/tls.test.ts src/lib/e-social/xml-linear.test.ts`
