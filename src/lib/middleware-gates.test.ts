...
describe('middleware matcher', () => {
  it('excludes api, static Next assets, and public file prefixes', () => {
    ...
    assert.equal(isExcludedByMiddlewareMatcher('/api/mobile/preview-disabled'), true);
...
describe('middleware public / static / lista-presenca', () => {
  it('treats login, register, set-password, reset and verify-email as public pages', () => {
    ...
    assert.equal(isPublicPagePath('/m'), true);
    assert.equal(isPublicPagePath('/m/login'), true);
    assert.equal(isPublicPagePath('/m/preview'), true);
  ...
  it('passes lista-presenca public links and static prefixes', () => {
    ...
    assert.equal(isAuthPassthroughPath('/api/mobile/preview-disabled'), true);
    assert.equal(isAuthPassthroughPath('/m/login'), true);
  });
});
...
describe('middleware branches', () => {
  ...
  it('gates only /avaliacao pages; other app routes stay locale-only', () => {
    ...
    assert.equal(classifyMiddlewarePath('/m/login', false), 'passthrough');
    assert.equal(classifyMiddlewarePath('/api/auth/login', false), 'matcher-excluded');
    assert.equal(classifyMiddlewarePath('/api/mobile/preview-disabled', false), 'matcher-excluded');
  });
});
