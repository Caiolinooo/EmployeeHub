...
- **Markdown das bolhas** (`renderChatMarkdown`): tabelas GFM (`| col |` + delimiter row) → `<table>` real com `overflow-x-auto`; `> ` → callout; listas bullet/numeradas com hanging indent (níveis por indentação; `- **Título**: desc` vira linha estruturada); headings `#`–`####`; `---` → hr; código fenced/inline mantido; links via `parseSafeChatLink` + `href` reconstruído com literal `https://`/`http://`/`mailto:` + `encodeURI` das partes — **nunca** texto cru no `href`, **nunca** `dangerouslySetInnerHTML`
...
- `npx tsx --test src/lib/ia/chat-href.test.ts`
