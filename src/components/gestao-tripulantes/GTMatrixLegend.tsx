...
export default function GTMatrixLegend({ className }: { className?: string }) {
  ...
  return (
    <div className={cn('flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs px-4 py-2.5 bg-white rounded-xl shadow-sm border border-gray-100 shrink-0', className)}>
      ...
      {LEGEND_ITEMS.map(item => (
        <div key={item.labelKey} className="flex items-center gap-1.5">
          <span className={`inline-block w-3 h-3 rounded-full shrink-0 ${item.color}`} />
