...
// Script CommonJS proposital (padrão scripts/apply-*.js do repo): roda com
// `node` puro, sem tsx/transpilação — require() é o mecanismo do padrão.
/**
 * Executor da migration de aceites de contracheque
 * (20260923_000002_contracheque_aceites.sql).
 * Padrão de scripts/apply-financeiro-core.js: lê SUPABASE env
 * (.env.local/.env/.env.production), aplica via pg direto quando possível e
 * cai para RPC exec_sql/execute_sql; no fim verifica o shape da tabela
 * (select de 1 linha via cliente service_role), o RLS (relrowsecurity) e a
 * ausência de policies.
 *
 * Idempotente: re-executar não falha (CREATE TABLE/INDEX IF NOT EXISTS, DO blocks).
 *
 * Uso: node scripts/apply-contracheque-aceites.js  → CONTRACHEQUE_ACEITES_OK
 */
const fs = require('fs');
const path = require('path');
const { createClient } = require('@supabase/supabase-js');

const MIGRATION = path.join(__dirname, '..', 'supabase', 'migrations', '20260923_000002_contracheque_aceites.sql');

const TABELAS = ['payroll_contracheque_aceites'];
  ...
  try {
    const { rows } = await client.query(`
      ...
      WHERE c.relnamespace = 'public'::regnamespace
        AND c.relname IN ('payroll_contracheque_aceites')
      ORDER BY c.relname
    `);
    return rows;
  } catch (e) {
...
async function main() {
  ...
  console.log('Applying contracheque aceites migration...');
  ...
  if (shapeOk) {
    console.log('CONTRACHEQUE_ACEITES_OK');
  } else {
