/**
 * Executor da migration Financeiro NFS-e (20260922_000002_financeiro_nfse.sql).
 ...
 * exec_sql/execute_sql; no fim verifica o shape das 3 tabelas (fin_municipios,
 * fin_nfse_config, fin_nfse_emissoes via cliente service_role), o RLS e o seed
 * de Macaé 3302403.
 *
 * Idempotente: re-executar não falha (CREATE TABLE/INDEX IF NOT EXISTS, seed ON CONFLICT DO NOTHING).
 *
 * Uso: node scripts/apply-financeiro-nfse.js
 */
const fs = require('fs');
const path = require('path');
const { createClient } = require('@supabase/supabase-js');

const MIGRATION = path.join(__dirname, '..', 'supabase', 'migrations', '20260922_000002_financeiro_nfse.sql');
...
async function verifyShape(supabase) {
  ...
  const chavePorTabela = {
    fin_municipios: 'codigo_ibge',
    fin_nfse_config: 'id',
    fin_nfse_emissoes: 'id',
  };
  for (const table of Object.keys(chavePorTabela)) {
    const { error } = await supabase.from(table).select(chavePorTabela[table]).limit(1);
    checks.push({ table, ok: !error, error: error?.message });
  }
  return checks;
}

async function verifyMacae(supabase) {
  const { data, error } = await supabase
    .from('fin_municipios')
    .select('codigo_ibge, nome, uf, provider_sugerido')
    .eq('codigo_ibge', MACAE_IBGE)
    .maybeSingle();
  if (error) return { ok: false, error: error.message };
  if (!data) return { ok: false, error: `Macaé ${MACAE_IBGE} ausente` };
  return { ok: true, macae: data };
}
  ...
  try {
    const { rows } = await client.query(`
      ...
      WHERE c.relnamespace = 'public'::regnamespace
        AND c.relname IN ('fin_municipios','fin_nfse_config','fin_nfse_emissoes')
      ORDER BY c.relname
    `);
    return rows;
  } catch (e) {
...
async function main() {
  ...
  console.log('Applying Financeiro NFS-e migration...');
  ...
  if (supabase) {
    ...
    const macae = await verifyMacae(supabase);
    if (macae.ok) {
      console.log(`  OK  seed Macaé: ${JSON.stringify(macae.macae)}`);
      macaeOk = true;
    } else {
      console.log(`  FAIL seed Macaé — ${macae.error}`);
    }
  ...
  if (shapeOk && macaeOk) {
    console.log('FIN_APPLY_NFSE_OK');
  } else {
    console.log('Done (sem confirmação de shape/seed — marker NÃO emitido).');
  }
}
