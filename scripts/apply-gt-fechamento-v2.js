/**
 * Executor da migration GT Fechamento v2 (20260916_000001_gt_fechamento_v2.sql).
 * Padrão de scripts/apply-gt-relatorios-migration.js: lê SUPABASE env
 * (.env.local/.env/.env.production), aplica via pg direto quando possível e
 * cai para RPC exec_sql/execute_sql; no fim verifica o shape das tabelas.
 *
 * Uso: node scripts/apply-gt-fechamento-v2.js
 */
const fs = require('fs');
const path = require('path');
const { createClient } = require('@supabase/supabase-js');

const MIGRATION = path.join(__dirname, '..', 'supabase', 'migrations', '20260916_000001_gt_fechamento_v2.sql');
...
async function verifyShape(supabase) {
  const checks = [];
  for (const table of ['gt_fechamento_periodos', 'gt_fechamento_marcacoes', 'gt_escala_edicoes']) {
    const { error } = await supabase.from(table).select('*').limit(1);
    checks.push({ table, ok: !error, error: error?.message });
  }
  const { error: aproErr } = await supabase
    .from('gt_relatorios_aprovacoes')
    .select('id, mes_referencia, data_inicio, data_fim, pendencias')
    .limit(1);
  checks.push({ table: 'gt_relatorios_aprovacoes(data_inicio,data_fim,pendencias)', ok: !aproErr, error: aproErr?.message });
  return checks;
}
  ...
  try {
    const { rows } = await client.query(`
      ...
      WHERE relnamespace = 'public'::regnamespace
        AND relname IN ('gt_fechamento_periodos','gt_fechamento_marcacoes','gt_escala_edicoes')
      ORDER BY relname
    `);
    return rows;
  } catch (e) {
...
async function main() {
  ...
  console.log('Applying GT Fechamento v2 migration...');
  ...
  console.log(applied || (supabase) ? 'Done.' : 'Nothing applied (sem pg e sem Supabase env).');
}
